const { Server, Message } = require('../model')
...
class NatsServer extends Server {
    ...
    _worker(delegate) {
        return async (msg, reply, topic) => {
            let message = JSON.parse(msg)
            try {
                let response = await delegate(message)
                this._client.publish(reply, JSON.stringify(response))
            } catch (e) {
                this._client.publish(reply, JSON.stringify(Message.newError(e)))
            }
        }
    }
